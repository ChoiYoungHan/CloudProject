    ...
    stage('Docker Build & Push') {
      steps {
        script {
          ...
          dir(folder) {
            container('kaniko') {
              sh "/kaniko/executor --context=. --dockerfile=Dockerfile --destination=207567776727.dkr.ecr.us-west-2.amazonaws.com/${repo}:${tag} --docker-config=/kaniko/.docker --verbosity=debug"
            }
          }
        }
      }
